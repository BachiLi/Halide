...
public:
    FlattenDimensions(const map<string, Parameter> &output_buffers,
                      const Target &t) : output_buffers(output_buffers), target(t) {}

private:
    const map<string, Parameter> &output_buffers;
    const Target &target;
    ...
    Stmt visit(const Realize *op) override {
        ...
        stmt = Allocate::make(op->name, op->types[0], op->memory_type, extents, condition, stmt);

        // Compute the strides
        for (int i = (int)op->bounds.size() - 1; i > 0; i--) {
            int prev_j = i - 1;
            int j = i;
            Expr stride = stride_var[prev_j] * extents[prev_j];
    ...
    Stmt visit(const Provide *op) override {
        ...
        auto it = output_buffers.find(op->name);
        if (it != output_buffers.end()) {
            output_buf = it->second;
        }
...
Stmt storage_flattening(Stmt s,
    ...
    s = FlattenDimensions(output_buffers, target).mutate(s);
