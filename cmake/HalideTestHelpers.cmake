...
function(tests)
    set(options EXPECT_FAILURE)
    set(oneValueArgs)
    set(multiValueArgs SOURCES GROUPS COMMAND_PREFIX ARGS COMMAND_SUFFIX)
    cmake_parse_arguments(args "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
    ...
    foreach (file IN LISTS args_SOURCES)
        ...
        if (args_EXPECT_FAILURE)
            add_halide_test("${TARGET}" COMMAND ${args_COMMAND_PREFIX} ${TARGET} ARGS ${args_ARGS} ${args_COMMAND_SUFFIX} GROUPS ${args_GROUPS} EXPECT_FAILURE)
            target_link_libraries("${TARGET}" PRIVATE Halide::ExpectAbort)
        else ()
            add_halide_test("${TARGET}" COMMAND ${args_COMMAND_PREFIX} ${TARGET} ARGS ${args_ARGS} ${args_COMMAND_SUFFIX} GROUPS ${args_GROUPS})
        endif ()
    endforeach ()
